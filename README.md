...
- `js/term-notes.js` が座学の各記事に出てきた用語集の用語を検出し、記事の下に解説ボックス(定義+用語集へのリンク)を自動表示します。データは `glossary.html` から `node tools/build-glossary-data.js` で生成される `js/glossary-data.js` です
- 主要な座学(初級・中級・上級)には、GP/LPの構造図・EVブリッジ・レバレッジ効果のグラフ・ウォーターフォールの図解など、テーマ色に追随するインラインSVGの図解と表を掲載しています
